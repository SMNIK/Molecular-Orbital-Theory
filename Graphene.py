...
class Graphene(Molecule):
    def __init__(self, *args):
        ...
        # Stores the row of hexagons that the most recentlygenerated carbon belongs to
        self.current_row = 1

    # m is number of hexagon rows, n is number of hexagon columns
    def generate_carbon(self, id, m, n, position_set):
    ...
    def zig_zipper(self, m, n):
        ...
        self.carbons[carb_iter +
                     1].pos = (pull_tab.pos[0], pull_tab.pos[1]-2*b)
    ...
    def plot_lattice(self, index):
        ...
        for i in range(self.num_carbons):
            for j in range(self.num_carbons):
                if self.H[i, j] == 1:
                    plt.plot([self.carbons[i].pos[0], self.carbons[j].pos[0]], [
                             self.carbons[i].pos[1], self.carbons[j].pos[1]])

        for c in self.carbons:
            circ = plt.Circle((c.pos[0], c.pos[1]),
                              c.psi_magnitudes[index] * 5)
            plt.gcf().gca().add_artist(circ)

        # Hide the axes in our plot
        plt.xticks([])    # hide the x-axes
        plt.yticks([])
        plt.title("Molecule Structure for " + self.name +
                  "with energy level " + str(index))
        plt.show()
